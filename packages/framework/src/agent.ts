...
export type Agent = {
  (state: WorkflowState, context: Message[], workflow: Workflow): Promise<WorkflowState>
  name?: string
  description?: string
  tools?: {
    [key: string]: Tool
...
export const agent = (options: AgentOptions): Agent => {
  const { role, description, tools, provider } = {
    ...defaults,
    ...options,
  }
  ...
  const run: Agent = async (state, context) => {
  ...
  Object.defineProperty(run, 'name', { value: role })
  Object.defineProperty(run, 'description', { value: description })
  Object.defineProperty(run, 'tools', { value: tools })
